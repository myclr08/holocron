"""Copilot CLI kopruusu: bul, cagir, cevabi DOSYADAN oku, sina.

Bu modul 0.11.0'da `app/gorusme/ozet.py` icinden cikarildi. Gorusme notlari
ozelligi tumden kaldirildi; ama Copilot'u bulmak, konsolsuz bir surecten
dogru cagirmak, kurumsal vekili yalnizca alt surece vermek ve "gercekten
calisiyor mu" diye sinamak genel isler. Ileride baska bir is Copilot'a
ihtiyac duyarsa hazir durur.

Cagri bicimi TDD Beyin kalibidir:

    copilot --model <model> -p "<istem>" --allow-tool=read --allow-tool=write

Cevap **dosyadan** alinir. Programatik kipte (`-p`) stdout'a banner, ilerleme
satirlari, arac kullanim dokumu ve ANSI renk kodlari karisir; cevap markdown
icinde ya da stderr'de kalabilir. Bu yuzden istemde modelden JSON'u bir
dosyaya yazmasi istenir, surec bitince o dosya okunur; dosya yoksa/bozuksa
YEDEK yol stdout + stderr'den ayiklamayi dener.

Model reddedilirse (sifir olmayan cikis kodu ya da ciktida taninabilir bir
hata) siradaki modele gecilir; calisan model "son calisan" olarak ayara
yazilir.
...
acildiktan sonra degismis olabilir).
"""
...
log = logging.getLogger("holocron.copilot")
...
# Modelin cevabi yazacagi dosya: calisma klasorune duser, okununca silinir.
CIKTI_DOSYASI = "copilot-cikti.json"

# Copilot'a verilen arac izinleri (TDD Beyin ile ayni bicim). `read` olmadan
# model verilen dosyayi okuyamaz, `write` olmadan cevabi dosyaya yazamaz.
...
def sessiz_calistir_ayarlari() -> dict[str, Any]:
    ...
    `start "" pythonw.exe` ile konsolsuz actiriyor. Boyle bir surec Copilot
    CLI'yi `subprocess` ile cagirdiginda Windows kendiliginden BOS bir konsol
    penceresi aciyor -- kullanicinin gordugu tek sey bu yaniltici pencere.

    Windows disinda bu bayraklar (CREATE_NO_WINDOW, STARTUPINFO) hic yok --
    `subprocess` modulunde bile tanimli degiller -- bu yuzden diger
    platformlarda sozluk yalnizca `stdin` tasir.

    `stdin=DEVNULL` HER platformda eklenir: konsolsuz surecte okunacak bir
    stdin zaten yok, ama bazi CLI'lar (Copilot dahil) girdi beklerse asilip
    kalabiliyor; DEVNULL onlari hemen EOF ile karsilar.
    """
...
def cikti_yolu(klasor: Path) -> Path:
    """Modelin cevabi yazacagi dosya.

    Istem de (`{cikti}`), okuyan taraf da bu tek fonksiyonu cagirir: yol iki
    yerde ayri ayri kurulup ayrisamaz.
    """
    return Path(klasor) / CIKTI_DOSYASI
...
# --- alt surecin ortami: vekil yalnizca orada ----------------------------
...
def bulunamadi_mesaji(denenen: Sequence[str]) -> str:
    ...
    return (
        ...
        + (", ".join(yerler) or "PATH")
        + ". Ayarlar → Copilot → Copilot yolu alanına `where copilot` "
        "çıktısını yazın."
    )
...
def komut_kur(
    ...
    olarak gecmez, calisma klasorune dosya olarak yazilir ve modele "o dosyayi
    oku" denir (`--allow-tool=read` zaten acik).

    `--allow-tool=write` de verilir: cevap stdout'tan degil modelin yazdigi
    dosyadan okunuyor, izin olmazsa dosya hic olusmaz.
    """
...
@dataclass
class CopilotCikti:
    """Copilot'un ham cevabi: kod sifir degilse model reddetmis olabilir.

    `dosya` BIRINCIL yoldur: model JSON'u stdout'a degil kendi yazdigi
    dosyaya koyar, orasi ANSI renk kodlarindan, ilerleme satirlarindan ve
    banner'dan temizdir. `metin` (stdout) ile `hata` (stderr) yedek ayiklama
    ve teshis icindir.
    """
...
class CopilotCalistirici:
    """Copilot CLI'yi alt surecte calistirir (testler `calistir`i degistirir)."""
    ...
    def sor(self, klasor: Path, model: str, istem: str) -> CopilotCikti:
        """Istemi calistirir; cevap once modelin yazdigi dosyadan okunur."""
        bulunan, denenen = copilot_coz(self.yol, self.komut)
        if bulunan is None:
            self.son_yol = ""
            return CopilotCikti(kod=127, metin="", hata=bulunamadi_mesaji(denenen))
        self.son_yol = str(bulunan)
        log.info("Copilot CLI bulundu: %s", bulunan)
        klasor = Path(klasor)
        klasor.mkdir(parents=True, exist_ok=True)
        hedef = cikti_yolu(klasor)
        ...
        try:
            ...
            return CopilotCikti(kod=1, metin="", hata=str(hata))
        finally:
...
def reddedildi_mi(cikti: Any) -> bool:
    """Model kullanilamiyor mu? (sifir olmayan kod ya da taninabilir hata)"""
    if int(getattr(cikti, "kod", 0) or 0) != 0:
        return True
    metin = f"{getattr(cikti, 'metin', '')}\n{getattr(cikti, 'hata', '')}".lower()
...
def veriyi_cek(cikti: Any) -> dict[str, Any] | None:
    """Copilot'un cevabindan JSON: once dosya, sonra stdout+stderr.

    Dosya birincil yoldur (temiz UTF-8). Model dosyayi yazamadiysa ama
    cevabi ekrana bastiysa sonuc yine de kurtarilir.
    """
...
def ayristir(metin: str) -> dict[str, Any] | None:
    ...
    kod bloklari ve dengeli suslu parantez kumeleri denenir. Model "İşte
    cevap:" gibi bir giris cumlesi yazsa ya da ilerleme satirlarinin arasina
    suslu parantezli bir sey dusse bile cevap kaybolmasin.
    """
...
# --- hata metinleri: sir maskeleme, kirpma, teshis -----------------------
...
def kuyruk(cikti: Any, sinir: int = HATA_SINIRI) -> str:
    """Copilot cevabinin (stdout+stderr) son `sinir` karakteri."""
...
def json_yok_mesaji(cikti: Any) -> str:
    ...
    Sahadaki "Model JSON döndürmedi." tek basina kullaniciya hicbir sey
    soylemiyordu; artik Copilot'un son sozleri de yaninda.
    """
...
def teshis_logla(model: str, cikti: Any) -> None:
    ...
    log.warning(
        "Copilot JSON'u çıkmadı (model %s, çıkış kodu %s). "
...
def sinama_istemi(cikti: Path) -> str:
    """Sinama istemi: gercek isin AYNI yolu, yalnizca kucucuk bir JSON.

    Gercek is cevabi dosyadan aliyor; sinama da ayni yoldan gecmezse
    "çalışıyor" der ama asil is yine patlar.
    """
    return (
...
def sina(calistirici: Any, modeller: Sequence[str], klasor: Path) -> dict[str, Any]:
    """Kisa bir istek atar: Copilot CLI ve vekil ayari gercekten calisiyor mu?

    Yedek sira burada da isler: ilk model reddederse siradaki denenir ve
    calisan model "son calisan" olarak geri bildirilir.

    Sinama gercek isin AYNI yolunu yurur: model kucuk bir JSON'u
    (`{"hazir": true}`) cikti dosyasina yazar, biz oradan okuruz. Boylece
    ...
    try:
        for model in modeller:
            ...
            bulunan_yol = str(getattr(calistirici, "son_yol", "") or "")
            parcalar = ["çalışıyor"]
            if bulunan_yol:
    ...
    return {
        "calisiyor": False,
        "model": "",
        "yol": str(getattr(calistirici, "son_yol", "") or ""),
        "sn": 0.0,
        "denenen": denenen,
...
# Ara dosyalarin durdugu klasor: veri dizininin altinda, tek bir yer.
CALISMA_KLASORU = "copilot"


@dataclass(frozen=True)
class Ayarlar:
    """Ayarlardan okunan Copilot yapilandirmasi."""
    ...
    def kok(self) -> Path:
        ...
        return paths.home_dir() / CALISMA_KLASORU

    def model_sirasi(self) -> list[str]:
        """Denenecek modeller; son calisan basa alinir."""
        return modelleri_coz(list(self.modeller), self.son_model)


def load_config(settings: Any) -> Ayarlar:
    """Ayar deposundan `copilot.*` degerlerini okur."""
    ham_modeller = settings.get("copilot.modeller", "") or ""
    modeller = modelleri_coz(ham_modeller)
    return Ayarlar(
        yolu=str(settings.get("copilot.yolu", "") or "").strip(),
        yolu_son=str(settings.get("copilot.yolu_son", "") or "").strip(),
        proxy=str(settings.get("copilot.proxy", "") or "").strip(),
